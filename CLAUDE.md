...
- 새 시각 방향(첫 페이지·새 레이아웃 언어)은 크게 만들기 전에 checkpoint로 작게 먼저 제출
  — 이미 승인된 방향의 반복은 제외(빠른 경로)
...
- **정식 경로 (위험·새 방향):** 위험 기능(결제·인증·권한·동시성), 새 도메인 로직,
  ...
  - 전체 절차: (필요시) spec → checkpoint → 구현 → 리뷰어 3종 → test-auditor.

## 필요할 때만 읽는 문서 (라우팅)
- UI/페이지 작업 시작 전: docs/design-rules.md (checkpoint로 승인된 기준)
