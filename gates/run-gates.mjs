...
function findProjectDirs() {
  ...
  return readdirSync(PROJECTS)
    ...
    .filter((p) => {
      try { return statSync(p).isDirectory() && existsSync(join(p, "src")); }
      catch { return false; }
    });
}
const projectDirs = findProjectDirs();
if (projectDirs.length === 0) {
  console.log("게이트 skip: projects/<이름>/src 아직 없음 (스캐폴드 전). 구조 생성: node scripts/scaffold.mjs <프로젝트명>");
  process.exit(0);
}
...
function firstUiFile(projDir) {
  for (const layer of ["pages", "widgets"]) {
    const hit = walk(join(projDir, "src", layer)).find((f) => /\.(ts|tsx|jsx|css|html)$/.test(f));
...
if (!QUICK) {
  ...
  for (const projDir of projectDirs) {
    ...
    if (existsSync(join(projDir, "tsconfig.json"))) {
      const r = spawnSync("npx", ["tsc", "--noEmit", "--pretty", "false"], { cwd: projDir, encoding: "utf-8", timeout: 120000, shell: true });
      if (r.error) {
        errors.push(`[tsc/SPAWN] ${label}: tsc 실행 실패: ${r.error.code} — Node에서 npx 실행 불가(shell 옵션 확인)`);
      } else if (r.status !== 0) {
        const out = (r.stdout ?? "") + (r.stderr ?? "");
        const re = /^(.+?)\((\d+),\d+\): error (TS\d+): (.+)$/gm;
        let m, found = false;
        while ((m = re.exec(out)) !== null) { errors.push(`[tsc/${m[3]}] ${label}/${m[1]}:${m[2]} — ${m[4]}`); found = true; }
    ...
    if (existsSync(pkgPath)) {
      ...
      if (pkg.scripts?.test) {
        const t = spawnSync("npm", ["test", "--silent"], { cwd: projDir, encoding: "utf-8", timeout: 300000, shell: true });
        if (t.status !== 0) errors.push(`[test/FAIL] ${label}: npm test 실패:\n${((t.stdout ?? "") + (t.stderr ?? "")).slice(-800)}`);
      }
    }
  }
  ...
  // 4) RLS 정책 커버리지: 앱이 쓰는 테이블에 그 동작의 정책이 있는지 (막힌 쓰기는 에러가 아니라 0행)
  const rls = spawnSync("node", [join(ROOT, "gates", "rls-coverage.mjs")], { cwd: ROOT, encoding: "utf-8" });
  if (rls.status !== 0) errors.push(...(rls.stderr ?? "").trim().split("\n").filter(Boolean));
}
...
console.log(`게이트 통과 (${fileCount}개 파일, ${projectDirs.length}개 프로젝트${QUICK ? ", quick" : ""})`);
