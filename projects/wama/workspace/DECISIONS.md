...
2026-08-06 | 시간표 슬롯의 시작·종료 시간을 **선택 입력**으로 (not null 해제). 단 넣으려면 둘 다 |
사용자가 실사용 리뷰에서: "시간이 유동적으로 변할 수 있어서 시간까지 입력하는 일이 적을 수도 있다."
학원 현장에서 요일·과목은 고정이지만 시각은 그 주 사정으로 흔들린다. 필수로 두면 선생님이 아무 값이나
넣게 되고(17:00 기본값 그대로 저장), 그러면 **틀린 시간이 맞는 것처럼 표에 박힌다** — 비어 있는 편이 정직하다 |
대안 '자유 텍스트(text 컬럼)' 기각 — "5시쯤" 같은 표기가 가능해지는 대신 시간순 정렬과 "종료가 시작보다
늦다" 검사를 영영 잃는다. 시간을 적을 때는 여전히 기계가 읽을 수 있어야 한다는 판단(사용자 선택).
대안 '한쪽만 입력 허용' 기각 — "17:00–" 같은 반쪽 표시가 화면마다 분기를 낳는다. DB CHECK 로
둘 다이거나 둘 다 아니거나만 허용한다. 수강료는 영향 없음 — 청구액은 주 횟수(슬롯 개수)로 계산되고
시각을 보지 않는다(INV-PN7 그대로). 마이그레이션 0013.
