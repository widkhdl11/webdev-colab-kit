...
- 오늘의 목표: 수강료 안내 이미지 기능 구현(4조각). **코드는 4조각 다 완료, 마지막 조각만 실동작 미확인.**
- 완료: 마이그레이션 0009~0012 적용 · 학원설정/과목가격표/학생별예외 3화면 **실동작 확인** · 내보내기 화면 +
  PNG 캡처(html2canvas) 코드 완성 · 버그 3건 수정(과목 갱신 누락 · 시간표 표시 어긋남 · academy UPDATE 정책 누락) ·
  파생 규칙 2건을 entities 로 이동(groupByMonth·pricesForSubject) · **RLS 커버리지 게이트 신설**(gates/rls-coverage.mjs) ·
  ui-layers 규칙 1줄. 테스트 152 green, 빌드·게이트 green.
- 멈춘 지점: **내보내기 화면(#/students/:id/notice)에서 PNG 저장을 눈으로 확인 못 함.** 코드·빌드·게이트는 통과.
  그리고 gates/run-gates.mjs 에 rls-coverage 호출을 붙이는 편집이 **디스크에 저장되지 않은 상태**(IDE 미저장 추정).
- 다음 할 일: 내보내기 화면에서 PNG를 실제로 저장해 폰트·금액·계좌번호가 이미지에서 깨지지 않는지 확인하고,
  그다음 리뷰어 3종(code·security·ui) + test-auditor 를 돌린다.
- 대기 중인 결정: **run-gates.mjs 에 rls-coverage 호출 저장**(안 하면 게이트가 안 돎) · wama 레포 푸시 여부(Vercel 배포) ·
  `/retro` 실행 여부(오늘 발견한 "같은 진실이 두 벌" 패턴을 LESSONS 에 남길지) · 이전 세션 미커밋 테스트 2개 ·
  스펙 4종 여전히 draft · 실배포 시 이메일확인 재활성.
